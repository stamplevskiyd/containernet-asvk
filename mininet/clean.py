    ...
    def cleanup(cls):
        ...
        for rule in chain.rules:
            if SAP_PREFIX in str(rule.out_interface):
                src_CIDR = str(ipaddress.IPv4Network("{}".format(str(rule.src))))
                rule0_ = "iptables -t nat -D POSTROUTING ! -o {0} -s {1} -j MASQUERADE".format(
                    rule.out_interface.strip("!"), src_CIDR
                )
                p = Popen(shlex.split(rule0_))
                p.communicate()
                info(
                    "delete NAT rule from SAP: {1} - {0} - {2}\n".format(
        ...
        for rule in chain.rules:
            ...
            if SAP_PREFIX in str(rule.out_interface):
                ...
                p.communicate()
                info(
                    "delete FORWARD rule from SAP: {1} - {0} - {2}\n".format(
                        rule.out_interface, rule.in_interface, src_CIDR
                    )
                )
