...
class Node(object):
    ...
    def sendCmd(self, *args, **kwargs):
        ...
        while self.waiting and cnt < 5 * 120:
            ...
            cnt += 1
        if cnt > 0:
            warn("Shell unblocked after {:.2f}s".format(float(cnt) / 5))
...
class CPULimitedHost(Host):
    ...
    def cgroupGet(self, param, resource="cpu"):
        ...
        cmd = "cgget -r %s.%s /%s" % (resource, param, self.name)
...
class OVSSwitch(Switch):
    ...
    def controllerUUIDs(self, update=False):
        ...
        if not self._uuids or update:
            controllers = self.cmd(
                "ovs-vsctl -- get Bridge", self.deployed_name, "Controller"
            ).strip()
    ...
    def start(self, controllers):
        ...
        intfs = "".join(
            " -- add-port %s %s" % (self.deployed_name, intf) + self.intfOpts(intf)
            for intf in self.intfList()
            if self.ports[intf] and not intf.IP()
        )
        # Command to create controller entries
        clist = [
            (self.deployed_name + c.name, "%s:%s:%d" % (c.protocol, c.IP(), c.port))
    ...
    def batchShutdown(cls, switches, run=errRun):
        ...
        run(
            "ovs-vsctl " + " -- ".join(delcmd % s.deployed_name for s in switches),
            shell=True,
        )
