...
class Mininet(object):
    ...
    def getNextIp(self):
        ip = (
            ipAdd(self.nextIP, ipBaseNum=self.ipBaseNum, prefixLen=self.prefixLen)
            + "/%s" % self.prefixLen
        )
        self.nextIP += 1
        return ip
    ...
    def ping(self, hosts=None, timeout=None, manualdestip=None):
        ...
        for node in hosts:
            ...
            if manualdestip is not None:
                ...
                if timeout:
                    opts = "-W %s" % timeout
                result = node.cmd("LANG=c ping -c1 %s %s" % (opts, manualdestip))
                ...
                for dest in hosts:
                    if node != dest:
                        ...
                        if dest.intfs:
                            result = node.cmd(
                                "LANG=C ping -c1 %s %s" % (opts, dest.IP())
                            )
                            sent, received = self._parsePing(result)
                        else:
                            sent, received = 0, 0
...
class Containernet(Mininet):
    ...
    def addExtSAP(self, sapName, sapIP, dpid=None, **params):
        ...
        :return:
        """
        SAPswitch = self.addSwitch(
            sapName, cls=OVSBridge, prefix=SAP_PREFIX, dpid=dpid, ip=sapIP, **params
        )
