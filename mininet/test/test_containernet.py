...
def find_test_container(filename=""):
    ...
    """
    abs_path = os.path.dirname(__file__).replace("mininet/test", "")
    return os.path.join(abs_path, "examples/example-containers/{}".format(filename))
...
class testImageBuilding(unittest.TestCase):
    def testaddDocker(self):
        ...
        d2 = net.addDocker(
            "d2",
            ip="10.0.0.252",
            build_params={"dockerfile": "Dockerfile.alpine3", "path": path},
        )
...
class simpleTestTopology(unittest.TestCase):
    ...
    def createNet(self, nswitches=1, nhosts=0, ndockers=0, autolinkswitches=False):
        """
...
class testContainernetConnectivity(simpleTestTopology):
    ...
    def testDockerMultipleInterfaces(self):
        ...
        self.net.addLink(self.d[1], self.s[1], params1={"ip": "11.0.0.1/8"})
...
class testContainernetDynamicTopologies(simpleTestTopology):
    ...
    def testSimpleRemove(self):
        ...
        self.assertTrue(
            self.net.ping([self.d[0]], manualdestip="10.0.0.2", timeout=1) >= 100.0
        )
        # stop Mininet network
        self.stopNet()
    ...
    def testFullyDynamic(self):
        ...
        # check connectivity by using ping
        self.assertTrue(
            self.net.ping([self.h[0]], manualdestip="10.0.0.200", timeout=1) >= 100.0
        )
        self.assertTrue(
            self.net.ping([self.h[0]], manualdestip="10.0.0.201", timeout=1) >= 100.0
        )
        ### add container: d4
        ...
        # check connectivity by using ping
        self.assertTrue(
            self.net.ping([self.h[0]], manualdestip="10.0.0.202", timeout=1) >= 100.0
        )
        self.assertTrue(
            self.net.ping([self.h[0]], manualdestip="10.0.0.203", timeout=1) >= 100.0
        )
        self.assertTrue(
            self.net.ping([self.h[0]], manualdestip="10.0.0.204", timeout=1) >= 100.0
        )
        # stop Mininet network
        self.stopNet()
...
class testContainernetTCLinks(simpleTestTopology):
    ...
    def testCustomLoss(self):
        ...
        self.net.addLink(self.s[0], self.d[0])
        self.net.addLink(self.s[0], self.d[1])
        self.net.addLink(self.s[0], self.d[2], cls=TCLink, loss=100)  # 100% loss
        # start Mininet network
        self.startNet()
        # check number of running docker containers
        self.assertTrue(len(self.getContainernetContainers()) == 3)
        ...
        # check connectivity by using ping: default link
        self.assertTrue(
            self.net.ping([self.d[0]], manualdestip="10.0.0.2", timeout=1) <= 0.0
        )
        # check connectivity by using ping: lossy TCLink (100%)
        self.assertTrue(
            self.net.ping([self.d[0]], manualdestip="10.0.0.3", timeout=1) >= 100.0
        )
        # stop Mininet network
        self.stopNet()
...
class testContainernetContainerResourceLimitAPI(simpleTestTopology):
    ...
    def testCPUShare(self):
        ...
        d0 = self.net.addDocker(
            "d0", ip="10.0.0.1", dimage="ubuntu:trusty", cpu_shares=10
        )
        d1 = self.net.addDocker(
            "d1", ip="10.0.0.2", dimage="ubuntu:trusty", cpu_shares=90
        )
        # setup links (we always need one connection to suppress warnings)
    ...
    def testMemLimits(self):
        ...
        # add dockers
        d0 = self.net.addDocker(
            "d0", ip="10.0.0.1", dimage="ubuntu:trusty", mem_limit=132182016
        )
        d1 = self.net.addDocker(
            "d1", ip="10.0.0.2", dimage="ubuntu:trusty", mem_limit=132182016
        )
        # setup links (we always need one connection to suppress warnings)
        self.net.addLink(d0, self.s[0])
        self.net.addLink(d1, self.s[0])
    ...
    @pytest.mark.skipif(
        os.environ.get("CONTAINERNET_NESTED") is not None,
    ...
    def testRuntimeCPULimitUpdate(self):
        ...
        # add dockers
        d0 = self.net.addDocker(
            "d0", ip="10.0.0.1", dimage="ubuntu:trusty", cpu_share=0.3
        )
        d1 = self.net.addDocker(
            "d1",
            ip="10.0.0.2",
...
class testContainernetVolumeAPI(simpleTestTopology):
    ...
    def testContainerVolumes(self):
        ...
        # add dockers
        d0 = self.net.addDocker(
            "d0", ip="10.0.0.1", dimage="ubuntu:trusty", volumes=["/:/mnt/vol1:rw"]
        )
        d1 = self.net.addDocker(
            "d1",
            ip="10.0.0.2",
...
@unittest.skip(
    "skip since storage_opt is only supported for overlay on XFS filesystems with pquota mount option"
)
class testContainernetContainerStorageOptAPI(simpleTestTopology):
    ...
    def testStorageOpt(self):
        ...
        # add dockers
        d0 = self.net.addDocker(
            "d0", ip="10.0.0.1", dimage="ubuntu:trusty", storage_opt={"size": "42m"}
        )
        d1 = self.net.addDocker(
            "d1", ip="10.0.0.2", dimage="ubuntu:trusty", storage_opt={"size": "1G"}
        )
        # setup links (we always need one connection to suppress warnings)
        self.net.addLink(d0, self.s[0])
        self.net.addLink(d1, self.s[0])
...
class testCustomTopologies(unittest.TestCase):
    ...
    def testNATWithTreeTopology(self):
        ...
        dockerClient.networks.create(
            "test-nat-network", driver="bridge", labels={"com.containernet": ""}
        )
        ...
        otherContainerIp = otherContainer.attrs["NetworkSettings"]["Networks"][
            "test-nat-network"
        ]["IPAddress"]
