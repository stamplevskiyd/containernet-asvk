...
class DockerDialog(CustomDialog):
    ...
    def body(self, master):
        ...
        self.ifaceButton = Button(
            self.ifaceFrame, text="add IP/subnet", command=self.addIpAndSubnet
        )
        self.ifaceButton.grid(row=0, column=1)

        self.ifaceFrame = VerticalScrolledTable(
            self.ifaceFrame, rows=0, columns=1, title="IPs/subnets for add. interfaces"
        )
    ...
    def apply(self):
        ...
        for row in range(self.ifaceTableFrame.rows):
            if len(self.ifaceTableFrame.get(row, 0)) > 0 and row > 0:
...
class MiniEdit(Frame):
    ...
    def __init__(self, parent=None, cheight=600, cwidth=1000):
        ...
        self.dockerPopup.add_command(
            label="Properties", font=self.font, command=self.dockerDetails
        )
        ...
        self.dockerRunPopup.add_command(
            label="Terminal", font=self.font, command=self.xterm
        )
    ...
    def dockerDetails(self, _ignore=None):
        ...
        prefDefaults = self.hostOpts[name]
        dockerBox = DockerDialog(
            self, title="Docker Details", prefDefaults=prefDefaults
        )
        ...
        if dockerBox.result:
            ...
            info(
                "New host details for " + name + " = " + str(self.hostOpts[name]), "\n"
            )
    ...
    def buildNodes(self, net):
        ...
        for widget in self.widgetToItem:
            ...
            if "Switch" in tags:
                ...
                newHost = net.addDocker(name, ip=ip, dimage=dimage, dcmd=startCommand)
        ...
        for key, link in self.links.items():
            ...
            if "data" in tags:
                ...
                if len(srcNode.intfList()) > 0 and srcName in self.hostOpts:
                    ...
                    if opts:
                        srcIP = None
                        if opts.get("multiInterfaces") and len(
                            opts.get("multiInterfaces")
                        ) >= len(srcNode.intfList()):
                            srcIP = opts.get("multiInterfaces")[
                                len(srcNode.intfList()) - 1
                            ]
                ...
                if len(dstNode.intfList()) > 0 and dstName in self.hostOpts:
                    ...
                    if opts:
                        ...
                        if opts.get("multiInterfaces") and len(
                            opts.get("multiInterfaces")
                        ) >= len(dstNode.intfList()):
                            dstIP = opts.get("multiInterfaces")[
                                len(dstNode.intfList()) - 1
                            ]
    ...
    def build(self):
        ...
        net = Containernet(topo=None, listenPort=dpctl, ipBase=self.appPrefs["ipBase"])
